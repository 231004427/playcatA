...
public class AreaListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    //构造函数
    public AreaListAdapter(List<Area> list,int selectId){
        this.mDatas = list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View layout = LayoutInflater.from(parent.getContext()).inflate(R.layout.listview_item_area, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(getItemViewType(position) == TYPE_NORMAL){
            if(holder instanceof ListHolder) {
                ...
                ImageView selectImg=((ListHolder) holder).selectImg;

                nameText.setText(info.getName());
                nameText.setTag(info);

                if(_selectId==info.getId()){
